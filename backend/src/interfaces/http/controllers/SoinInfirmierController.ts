...
export class SoinInfirmierController {
  ...
  create = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      notificationService.notifyAdmins({
        titre:   'Nouveau soin infirmier',
        message: `Soin infirmier enregistré par ${auteur} pour le patient #${validatedData.id_patient}`,
        type: 'info', priorite: 'normale', lien: lienAuteur,
      }).catch(console.error);

      notifyMedecinTraitant(validatedData.id_patient, role, {
        titre:   'Nouveau soin infirmier sur votre patient',
        message: `${auteur} a enregistré un soin infirmier pour le patient #${validatedData.id_patient}`,
        type: 'info', priorite: 'normale', lien: lienMedecin,
      }).catch(console.error);

      res.status(201).json({ success: true, message: 'Soin infirmier créé avec succès', data: soin });
    } catch (error) {
  ...
  getById = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!soin) throw new NotFoundError('Soin infirmier non trouvé');
  ...
  update = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!existingSoin) throw new NotFoundError('Soin infirmier non trouvé');
  ...
  verify = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: `Soin infirmier ${soin.verifie ? 'vérifié' : 'non vérifié'}`, data: soin });
    } catch (error) {
  ...
  delete = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: 'Soin infirmier supprimé avec succès' });
    } catch (error) {
      if (error instanceof NotFoundError) { res.status(404).json({ success: false, message: error.message }); return; }
