// backend/src/interfaces/http/controllers/SoinMedicalExportController.ts
...
const soinRepo = new PostgresSoinMedicalRepository(pool);
const patientRepo = new PostgresPatientRepository(pool);
const pdfService = new SoinMedicalPDFService();

export class SoinMedicalExportController {
  ...
  async downloadPDF(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=soin_medical_${id}_${patient.nom_patient}.pdf`
      );
  ...
  async downloadAllZIP(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=soins_medicaux_${patient.nom_patient}_${patient.prenom_patient}.zip`
      );
      ...
      for (const [index, soin] of soins.entries()) {
        ...
        // Ajouter au ZIP
        const fileName = `soin_medical_${index + 1}_${new Date(soin.date_soin).toLocaleDateString('fr-FR').replace(/\//g, '-')}.pdf`;
