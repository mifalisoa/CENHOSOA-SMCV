...
export class ObservationController {
  ...
  create = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const lienAuteur  = getDossierLien(role, data.id_patient);
      const lienMedecin = getDossierLienMedecin(data.id_patient);

      notificationService.notifyAdmins({
        titre:   'Nouvelle observation médicale',
        message: `Observation créée par ${auteur} pour le patient #${data.id_patient}`,
        type: 'info', priorite: 'normale', lien: lienAuteur,
      }).catch(console.error);

      notifyMedecinTraitant(data.id_patient, role, {
        titre:   'Nouvelle observation sur votre patient',
        message: `${auteur} a créé une observation médicale pour le patient #${data.id_patient}`,
        type: 'info', priorite: 'normale', lien: lienMedecin,
      }).catch(console.error);

      res.status(201).json({ success: true, message: 'Observation créée avec succès', data: observation });
    } catch (error) {
      ...
      res.status(500).json({ success: false, message: "Erreur serveur lors de la création de l'observation" });
    }
  };
  ...
  getById = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!observation) throw new NotFoundError('Observation non trouvée');
  ...
  update = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: 'Observation mise à jour avec succès', data: observation });
    } catch (error) {
      if (error instanceof ZodError)      { res.status(400).json({ success: false, message: 'Erreur de validation', errors: error.issues }); return; }
      if (error instanceof NotFoundError) { res.status(404).json({ success: false, message: error.message }); return; }
  ...
  delete = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: 'Observation supprimée avec succès' });
    } catch (error) {
      if (error instanceof NotFoundError) { res.status(404).json({ success: false, message: error.message }); return; }
