...
export class CompteRenduController {
  ...
  create = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(201).json({ success: true, message: 'Compte rendu créé avec succès', data: compteRendu });
    } catch (error) {
  ...
  getByAdmissionId = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!compteRendu) { res.status(404).json({ success: false, message: 'Aucun compte rendu trouvé pour cette admission' }); return; }
  ...
  getById = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!compteRendu) throw new NotFoundError('Compte rendu non trouvé');
      res.status(200).json({ success: true, data: compteRendu });
    } catch (error) {
  ...
  update = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: 'Compte rendu mis à jour avec succès', data: compteRendu });
    } catch (error) {
      if (error instanceof ZodError)      { res.status(400).json({ success: false, message: 'Erreur de validation', errors: error.issues }); return; }
      if (error instanceof NotFoundError) { res.status(404).json({ success: false, message: error.message }); return; }
  ...
  delete = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: 'Compte rendu supprimé avec succès' });
    } catch (error) {
      if (error instanceof NotFoundError) { res.status(404).json({ success: false, message: error.message }); return; }
