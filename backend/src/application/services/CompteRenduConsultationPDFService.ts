...
import type { CompteRenduConsultation } from '../../domain/entities/CompteRenduConsultation';
import type { Patient } from '../../domain/entities/Patient';

export class CompteRenduConsultationPDFService {
  generatePDF(cr: CompteRenduConsultation, patient: Patient): PDFKit.PDFDocument {
    ...
    this.addSection(doc, 'MOTIF DE CONSULTATION', cr.motif_consultation);
    ...
    this.addSection(doc, 'TRAITEMENT', cr.traitement);
  ...
  // Identique au service hospitalisation — voir CompteRenduPDFService.ts pour le detail
  // du raisonnement. Necessaire ici aussi car le texte colle depuis Word contient des
  // caracteres que la police standard PDFKit (WinAnsi) ne sait pas afficher.
  ...
  private addOfficialHeader(doc: PDFKit.PDFDocument, cr: CompteRenduConsultation, patient: Patient) {
    const startY    = 20;
    const leftColX  = 40;
    const rightColX = 280;
    ...
    doc.fontSize(15).font('Helvetica-Bold')
       .text('COMPTE RENDU DE CONSULTATION EXTERNE', rightColX, startY + 10, { underline: true, width: 260 });
    ...
    // Date de consultation — une seule date, pas de plage comme pour l'hospitalisation
    const dateY = patientY + 60;
    const dateConsultation = new Date(cr.date_consultation);
    doc.text(`DATE DE CONSULTATION : ${dateConsultation.toLocaleDateString('fr-FR')}`, rightColX, dateY);

    doc.moveTo(50, 255).lineTo(545, 255).dash(2, { space: 2 }).stroke().undash();
    doc.y = 270;
  }

  // Bloc statique — identique au service hospitalisation, voir CompteRenduPDFService.ts
  ...
  private addSignature(doc: PDFKit.PDFDocument, cr: CompteRenduConsultation) {
    if (doc.y > 680) doc.addPage();

    const date  = new Date(cr.date_consultation);
