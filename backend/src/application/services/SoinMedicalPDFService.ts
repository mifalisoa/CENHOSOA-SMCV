...
export class SoinMedicalPDFService {
  generatePDF(soin: SoinMedical, patient: Patient): PDFKit.PDFDocument {
    ...
    // 2. Contenu des soins
    if (soin.ett) {
      this.addSection(doc, 'ÉCHOCARDIOGRAPHIE TRANSTHORACIQUE (ETT)', soin.ett);
    }
    
    if (soin.eto) {
      this.addSection(doc, 'ÉCHOCARDIOGRAPHIE TRANSŒSOPHAGIENNE (ETO)', soin.eto);
    }
    
    if (soin.autre) {
      this.addSection(doc, 'AUTRE SOIN MÉDICAL', soin.autre);
    }
