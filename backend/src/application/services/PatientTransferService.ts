...
//
//  les services applicatifs ne doivent JAMAIS
// hardcoder des données métier. L'identité de l'acteur vient toujours du
// contexte d'exécution (JWT, session) passé explicitement en paramètre.
...
export class PatientTransferService {
  ...
  private async generateNumAdmission(
    client: { query: (q: string, p?: unknown[]) => Promise<{ rows: Record<string, string>[] }> }
  ): Promise<string> {
    const year   = new Date().getFullYear();
    const result = await client.query(
      `SELECT num_admission FROM admission
       WHERE num_admission LIKE $1
       ORDER BY num_admission DESC LIMIT 1`,
      [`A${year}%`]
    );
    if (result.rows.length === 0) return `A${year}0001`;
    const last = parseInt(result.rows[0].num_admission.slice(-4));
    return `A${year}${(last + 1).toString().padStart(4, '0')}`;
  }
  ...
  async hospitaliserPatient(data: HospitalisationData): Promise<Patient> {
    ...
    try {
      ...
      // 2. Vérifier qu'il n'a pas déjà une admission active
      const admissionActive = await client.query(
        `SELECT id_admission FROM admission
         WHERE id_patient = $1 AND statut_admission = 'en_cours'`,
        [data.id_patient]
      );
      if (admissionActive.rows.length > 0) {
        throw new Error('Le patient a déjà une admission active en cours');
      }

      // 3. Créer l'admission avec le vrai médecin connecté
      ...
      const admissionResult = await client.query(
        `INSERT INTO admission
           (id_patient, id_docteur, num_admission, date_admission,
            motif_admission, diagnostic_entree, type_admission,
            statut_admission, id_lit)
         VALUES ($1, $2, $3, $4, $5, $6, $7, 'en_cours', $8)
         RETURNING *`,
        [
          data.id_patient,
          data.id_medecin,          // ✅ Vrai médecin, plus jamais "1"
          numAdmission,
          dateAdmission,
          data.motif_hospitalisation,
          data.service_hospitalisation,
          typeAdmission,
          data.id_lit || null,
        ]
      );
      ...
      // 4. Occuper le lit si assigné
      if (data.id_lit) {
        await client.query(
          `UPDATE lit SET statut_lit = 'occupe' WHERE id_lit = $1`,
          [data.id_lit]
        );
      }

      // 5. Passer le patient en hospitalisé
      const updatedPatient = await client.query(
        `UPDATE patients SET statut_patient = 'hospitalise'
         WHERE id_patient = $1 RETURNING *`,
        [data.id_patient]
      );

      await client.query('COMMIT');
      console.log(
        `✅ Patient ${patient.nom_patient} hospitalisé` +
        ` par médecin #${data.id_medecin}` +
        ` (admission ${admission.num_admission})`
      );
  ...
  async rendrePatientExterne(data: SortieData): Promise<Patient> {
    ...
    try {
      ...
      await client.query(
        `UPDATE admission
         SET date_sortie_prevue    = $1,
             remarques_admission   = $2,
             statut_admission      = 'sortie'
      ...
      if (admission.id_lit) {
        await client.query(
          `UPDATE lit SET statut_lit = 'disponible' WHERE id_lit = $1`,
          [admission.id_lit]
        );
      }

      const updatedPatient = await client.query(
        `UPDATE patients SET statut_patient = 'externe'
         WHERE id_patient = $1 RETURNING *`,
        [admission.id_patient]
      );
