...
export class PostgresObservationRepository implements IObservationRepository {
  ...
  async findById(id: number): Promise<Observation | null> {
    const result = await this.pool.query(
      'SELECT * FROM observation WHERE id_observation = $1', [id]
    );
  ...
  async findByPatientId(patientId: number, type?: 'externe' | 'hospitalise'): Promise<Observation[]> {
    let query = 'SELECT * FROM observation WHERE id_patient = $1';
    const params: unknown[] = [patientId];
    if (type) { query += ' AND type_observation = $2'; params.push(type); }
  ...
  async findByAdmissionId(admissionId: number): Promise<Observation[]> {
    const result = await this.pool.query(
      `SELECT * FROM observation WHERE id_admission = $1
  ...
  async delete(id: number): Promise<void> {
    await this.pool.query('DELETE FROM observation WHERE id_observation = $1', [id]);
  }
  ...
  private mapRowToObservation(row: any): Observation {
    return {
      ...
      type_observation:                row.type_observation as 'externe' | 'hospitalise',
      ...
      antecedents_cmo:                 row.antecedents_cmo  as Observation['antecedents_cmo'],
      antecedents_gmo:                 row.antecedents_gmo  as Observation['antecedents_gmo'],
      antecedents_che:                 row.antecedents_che  as Observation['antecedents_che'],
      examen_general:                  row.examen_general               as Observation['examen_general'],
      examen_physique_central:         row.examen_physique_central      as Observation['examen_physique_central'],
      examen_physique_peripherique:    row.examen_physique_peripherique as Observation['examen_physique_peripherique'],
