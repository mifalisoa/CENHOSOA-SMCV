...
const getRoleColor = (role: string) => {
  switch (role) {
    ...
    case 'interne':    return 'bg-cyan-100   text-cyan-700   border-cyan-300';
    case 'stagiaire':  return 'bg-sky-100    text-sky-700    border-sky-300';
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button onClick={loadUtilisateurs} disabled={loading}
                className="flex items-center gap-2 bg-white text-cyan-600 px-4 py-2 rounded-lg font-semibold hover:bg-gray-100 transition-colors">
                <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />Actualiser
              </button>
              <button onClick={handleCreate}
                className="flex items-center gap-2 bg-white text-cyan-600 px-5 py-2 rounded-lg font-semibold hover:bg-gray-100 transition-colors">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className="bg-white/15 rounded-lg p-3">
            ...
            {filteredUsers.map(user => {
              ...
              return (
                <div key={user.id_user}
                  className="bg-white rounded-xl p-5 border-2 border-gray-200 hover:border-cyan-400 transition-all hover:shadow-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="pt-2 border-t border-gray-200">
                      <p className="text-xs font-semibold text-gray-500 mb-1">
                        {user.role === 'admin' ? 'Tous les droits' : `${permissions.length} permissions par défaut`}
                      </p>
                      {user.role !== 'admin' && (
                        <div className="flex flex-wrap gap-1">
                          {permissions.slice(0, 3).map((perm: string) => (
                            <span key={perm} className="px-2 py-0.5 bg-cyan-50 text-cyan-700 rounded text-xs">
