...
export default function PatientDossierPage() {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      ...
      try {
        ...
        if (data) {
          setPatient(data);
          if (data.statut_patient === 'hospitalise' || data.statut_patient === 'hospitalisé') {
            await fetchLitActuel(parseInt(id));
          }
        } else {
  ...
  const fetchLitActuel = async (patientId: number) => {
    try {
      const litsResponse = await httpClient.get('/lits');

      interface LitWithPatient {
        id_lit: number;
        numero_lit: string;
        categorie: string;
        statut: string;
      ...
      const litOccupe = litsResponse.data.find((lit: LitWithPatient) =>
        lit.statut === 'occupe' && lit.patient_actuel?.id_patient === patientId
      );

      if (litOccupe) {
        setLitActuel({
          id_lit: litOccupe.id_lit,
          numero_lit: litOccupe.numero_lit,
          categorie: litOccupe.categorie,
        });
      }
    } catch (error) {
      console.error('Erreur chargement lit actuel:', error);
    }
  };
  ...
  const isHospitalise = patient.statut_patient === 'hospitalisé' || patient.statut_patient === 'hospitalise';
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="hidden md:flex items-center gap-2 text-sm text-gray-600">
        ...
        <span>›</span>
        <button
          onClick={() => navigate(isHospitalise ? '/patients-hospitalises' : '/patients-externes')}
        ...
        <div className="flex items-center justify-between gap-4 text-white">
          <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
            ...
            <div
              className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center text-lg md:text-xl font-bold flex-shrink-0 ${
                isHospitalise ? 'bg-cyan-700/60' : 'bg-white/20'
              }`}
            >
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm min-w-0">
              ...
              <span>•</span>
              <span className="flex items-center gap-1">{calculateAge(patient.date_naissance)} ans</span>
              <span>•</span>
          ...
          <div className="flex gap-2 flex-shrink-0">
            ...
            <button
              ...
              className="hidden md:flex px-4 py-2 bg-blue-700 hover:bg-blue-800 rounded-lg transition-all text-sm font-medium shadow-md items-center gap-2 disabled:opacity-50 whitespace-nowrap"
            >
              {downloadingDossier ? (
                <>
            ...
            <button
              onClick={handleDownloadAllDossier}
              disabled={downloadingDossier}
              className="md:hidden p-2 bg-blue-700 hover:bg-blue-800 rounded-lg transition-all shadow-md disabled:opacity-50"
            ...
            {/* Bouton Changer de lit */}
            {isHospitalise && litActuel && (
              <button
                onClick={() => setShowTransfererLitModal(true)}
                className="hidden md:flex px-4 py-2 bg-cyan-700 hover:bg-cyan-800 rounded-lg transition-all text-sm font-medium shadow-md items-center gap-2 whitespace-nowrap"
                title="Transférer vers un autre lit"
              >
