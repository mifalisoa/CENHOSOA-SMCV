...
export function LitManagement({ onBackToDashboard }: LitManagementProps) {
  ...
  const grouperParChambre = (litsAGrouper: Lit[]): Chambre[] => {
    ...
    litsAGrouper.forEach(lit => {
      ...
      if (!chambresMap.has(nomChambre)) {
        chambresMap.set(nomChambre, { nom: nomChambre, categorie: lit.categorie, capacite: 0, lits: [], etage: lit.etage });
      }
  ...
  const getLitLabel = (lit: Lit): string => {
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              />
              {searchPatient && (
                <button onClick={() => setSearchPatient('')} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600">✕</button>
              )}
            </div>
                  ...
                  >
                    <CardContent className="p-3">
                      ...
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-[#08C5D1] rounded-full transition-all duration-500" style={{ width: `${occupancyRate}%` }}></div>
                      </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {[
                ...
                <Button
                  key={label}
                  variant="outline"
                  onClick={() => setSelectedStatut(value)}
                  className={`${selectedStatut === value ? 'bg-white text-[#08C5D1] ring-2 ring-white' : 'bg-white/20 text-white hover:bg-white/30'} border-0 text-sm`}
                >
                  {label}
                </Button>
              ))}
            ...
            <div className="text-white/90 text-sm">
              ...
              {(selectedCategorie || selectedStatut || searchPatient) && (
                <button onClick={() => { setSelectedCategorie(null); setSelectedStatut(null); setSearchPatient(''); }} className="ml-3 underline hover:text-white">
                ...
                <div className="p-4 space-y-3">
                  {chambre.lits.map(lit => {
                    ...
                    return (
                      ...
                      >
                        {/* Bande colorée en haut pour les lits occupés */}
                        {isOccupe && (
                          <div className="h-1 bg-gradient-to-r from-cyan-400 to-blue-500 w-full" />
                        )}
                        ...
                        <div className={`p-3 ${isOccupe ? 'bg-white' : ''}`}>
                          ...
                          {isOccupe && lit.patient_actuel ? (
                            /* Bloc patient — fond cyan très léger, texte foncé lisible */
