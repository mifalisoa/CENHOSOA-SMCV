// frontend/src/presentation/components/patients/tabs/QuickSoinInfirmierAttachmentButton.tsx
...
import type { CreateSoinInfirmierDTO, SoinInfirmier } from '../../../../core/entities/SoinInfirmier';
import type { TypePieceJointe } from '../../../../core/entities/PieceJointe';
import { toast } from 'sonner';

interface QuickSoinInfirmierAttachmentButtonProps {
  patientId:  number;
  createSoin: (data: CreateSoinInfirmierDTO) => Promise<SoinInfirmier | null>;
}

export default function QuickSoinInfirmierAttachmentButton({ patientId, createSoin }: QuickSoinInfirmierAttachmentButtonProps) {
  const { user } = useAuth();
  const [processing, setProcessing] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const { createPieceJointe } = usePiecesJointes('soin_infirmier', undefined);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const nouveauSoin = await createSoin({
        ...
        autre_soins: 'Pièce jointe rapide',
      ...
      await createPieceJointe({
        entite_type:    'soin_infirmier',
        entite_id:       nouveauSoin.id_soin_infirmier,
        url_fichier,
        nom_fichier,
