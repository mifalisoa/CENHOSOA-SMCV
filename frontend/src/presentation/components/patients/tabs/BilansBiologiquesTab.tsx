...
import { Plus, Beaker, Calendar, Clock, User, Building2, Download, FileArchive } from 'lucide-react';
...
export default function BilansBiologiquesTab({ patient }: BilansBiologiquesTabProps) {
  ...
  const handleDownloadPDF = async (bilanId: number) => {
    ...
    try {
      const response = await httpClient.get(`/bilans-biologiques/${bilanId}/pdf`, {
        responseType: 'blob',
      });
  ...
  // Télécharger tous les bilans en ZIP
  const handleDownloadAllZIP = async () => {
    if (bilans.length === 0) {
      toast.error('Aucun bilan à télécharger');
      return;
    }

    setDownloadingAll(true);
    try {
      const response = await httpClient.get(`/bilans-biologiques/patient/${patient.id_patient}/zip`, {
        responseType: 'blob',
      });
