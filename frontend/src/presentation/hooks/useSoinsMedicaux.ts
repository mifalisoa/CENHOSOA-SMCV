...
import { SoinMedicalRepository } from '../../infrastructure/repositories/SoinMedicalRepository';
import type { SoinMedical, CreateSoinMedicalDTO } from '../../core/entities/SoinMedical';

const soinRepository = new SoinMedicalRepository();

export const useSoinsMedicaux = (patientId?: number) => {
  const [soins, setSoins] = useState<SoinMedical[]>([]);
  ...
  const createSoin = async (data: CreateSoinMedicalDTO): Promise<SoinMedical | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const deleteSoin = async (id: number): Promise<boolean> => {
    ...
    try {
      ...
      setSoins(prev => prev.filter(soin => soin.id_soin_medical !== id));
      return true;
    } catch (err: unknown) {
